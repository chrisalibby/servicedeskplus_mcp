...
| `delete_solution` | Unresolved: `DELETE /solutions/{id}` consistently returns 400 "Not in trash" even on a freshly created article — no `/solutions/{id}/move_to_trash` (or `/trash`, `/movetotrash`) sub-route exists (404), and bulk `DELETE /solutions` with an ids body 400s ("Extra parameter(s) not allowed"). No working delete path found live 2026-08-01; leftover `MCP TEST SOLUTION` / `[TEST] integration probe` articles (ids 144–146) need manual cleanup via the SDP UI. |
...
| `add_project_member` | Passing a technician's `email_id` was observed to add a different technician as the member in one live test — root cause unconfirmed; verify the returned member matches expectations before relying on it. |
